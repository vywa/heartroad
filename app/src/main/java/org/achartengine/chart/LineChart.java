...
public class LineChart extends XYChart {
	 ...
	 */
	@Override
	public void drawSeries(Canvas canvas, Paint paint, List<Float> points, List<Double> values, XYSeriesRenderer renderer, SupportSeriesRender supportSeriesRender, float yAxisValue,
		...
		for (FillOutsideLine fill : fillOutsideLine) {
			if (fill.getType() != FillOutsideLine.Type.NONE) {
				...
				if (fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE || fill.getType() == FillOutsideLine.Type.BOUNDS_BELOW) {
					...
					boolean add = false;
					if (fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE && fillPoints.get(1) < referencePoint || fill.getType() == FillOutsideLine.Type.BOUNDS_BELOW
					...
					for (int i = 3; i < fillPoints.size(); i += 2) {
						...
						if (prevValue < referencePoint && value > referencePoint || prevValue > referencePoint && value < referencePoint) {
							...
							if (fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE && value > referencePoint || fill.getType() == FillOutsideLine.Type.BOUNDS_BELOW
							...
							if (add || fill.getType() == FillOutsideLine.Type.BOUNDS_ABOVE && value < referencePoint || fill.getType() == FillOutsideLine.Type.BOUNDS_BELOW
									&& value > referencePoint) {
	...
	public void drawMyView(){
		if (canvas!= null && points!=null) {
			List<Float> myPoints = new ArrayList<Float>();
			myPoints.addAll(points);

			ArrayList<Float> ps = new ArrayList<Float>();

			Paint myPaint = new Paint();
			myPaint.setColor(Color.parseColor("#99f44d70"));
			myPaint.setStyle(Style.FILL);

			for (int i = 3; i < myPoints.size(); i += 2) {
				ps.clear();
				float prevValue = myPoints.get(i - 2);
				float value = myPoints.get(i);

				if (prevValue < myTarget && value < myTarget) {
					continue;
				}
				float prevX = myPoints.get(i - 3);
				float x = myPoints.get(i - 1);
				...
				if (prevValue < myTarget && value > myTarget) {
					...
					ps.add(x);
					ps.add(myTarget);
					ps.add(prevX + (x - prevX) * (myTarget - prevValue) / (value - prevValue));
					ps.add(myTarget);

				} else if (prevValue > myTarget && value < myTarget) {
				...
				drawMyPath(canvas, ps, myPaint);
			}
			
		}
	}
	
	void drawMyPath(Canvas canvas, List<Float> points, Paint paint) {
		Path path = new Path();
		path.moveTo(points.get(0), points.get(1));// 此点为多边形的起点
		for (int i = 3; i < points.size(); i += 2) {
			path.lineTo(points.get(i-1), points.get(i));
		}
	...
	protected ClickableArea[] clickableAreasForPoints(List<Float> points, List<Double> values, float yAxisValue, int seriesIndex, int startIndex) {
		...
		for (int i = 0; i < length; i += 2) {
			...
			ret[i / 2] = new ClickableArea(new RectF(points.get(i) - selectableBuffer, points.get(i + 1) - selectableBuffer, points.get(i) + selectableBuffer, points.get(i + 1)
	...
	public void drawSpecifiedPoint(Canvas canvas, SupportSeriesRender supportSeriesRender, Point point, String description) {
		...
		case SHOW_BOX: {
			...
			// TODO
			// Log.wtf("xxxxxxxxxx", "000000000000000");
		}
			break;
