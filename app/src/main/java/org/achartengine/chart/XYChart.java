...
public abstract class XYChart extends AbstractChart {
    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
        ...
        if (mRenderer.isTargetLineVisible()) {
        	for (int i = 0; i < mRenderer.getTargetSize(); i++) {
        		paint.setStyle(Style.FILL);
                float targetValue = mRenderer.getTargetValue(i);
                float targetHeight = (float) (bottom - yPixelsPerUnit[0] * (targetValue - minY[0]));
                paint.setStrokeWidth(DensityUtils.dp2px(App.getContext(), 0.75f));

                paint.setColor(mRenderer.getTargetLineColor(i));
                
                if (i == 3) {
                	myTarget = targetHeight;
                	drawMyView();
				}
                
                float targetTextWidth = 0;
                float targetTextHeight = 0;
                if (!"".equals(mRenderer.getTargetDescription(i))) {
                	paint.setTextSize(DensityUtils.dp2px(App.getContext(), 12));
                    targetTextWidth = 5 + paint.measureText(mRenderer.getTargetDescription(i));
...
//                 float textY = targetHeight+ targetTextHeight / 3 ;
                    float textY = targetHeight - DensityUtils.dp2px(App.getContext(), 3f);
                    if (i ==0) {
                    	lastDesPostion = 0;
					}
//                    Log.wtf("YYYYYYYYYYYYY", mRenderer.getTargetDescription(i));
                    if (lastDesPostion > targetHeight - targetTextHeight) {//有重叠
                    	textY = targetHeight + targetTextHeight/2;					//放在虚线以下
//                    	Log.wtf("xxxxxxxx", mRenderer.getTargetDescription(i));
                    }
                    
                    lastDesPostion = targetHeight;
                    canvas.drawText(mRenderer.getTargetDescription(i), textX, textY, paint);
                }
...
//                        path.lineTo(canvas.getWidth() - targetTextWidth, targetHeight);
                        path.lineTo(canvas.getWidth() , targetHeight);
                        int j = DensityUtils.dp2px(App.getContext(), 5);
                        PathEffect effects = new DashPathEffect(new float[]{j,j,j,j}, 1);
    ...
    public float myTarget = 2000;
    public void drawMyView(){
    }
    
    float lastDesPostion = 0f;
...
//                    }
    ...
    protected void drawPopupBox(Canvas canvas, Point point, int color, String title) {
        ...
        int rWidth = (int) (maxTextWidth * 1.05);
        int rHeight = (int) (maxTextHeight * titles.length);

        float pointHeadY;
        float pointFootY;
        if (point.getY() - (rHeight / 2) - 50 < 0) {
        	pointHeadY = 0;
        	pointFootY = (point.getY() + (rHeight / 2) - 50) - (point.getY() - (rHeight / 2) - 50);
		}else{
			pointHeadY = point.getY() - (rHeight / 2) - 50;
			pointFootY = point.getY() + (rHeight / 2) - 50;
		}
        //TODO 调整弹出框位置
