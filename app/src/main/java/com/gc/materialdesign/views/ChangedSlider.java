...
public class ChangedSlider extends CustomView {
	...
	public ChangedSlider(Context context, AttributeSet attrs) {
		super(context, attrs);
		setAttributes(attrs);
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (isEnabled()) {
			if (event.getAction() == MotionEvent.ACTION_DOWN || event.getAction() == MotionEvent.ACTION_MOVE) {
				...
				float x = v*(ball.xFin - ball.xIni) / (max - min)+DensityUtils.dp2px(App.getContext(), 10);
				ViewHelper.setX(ball, x);
				ball.changeBackground();
	...
	class Indicator extends RelativeLayout {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (animate) {
				...
				y -= Utils.dpToPx(18, getResources());
				size += Utils.dpToPx(6, getResources());
			}
			...
			if (animate == false) {
				...
				switch (kind) {
				case 0:
					numberIndicator.numberIndicator.setText("凌晨\n测量");
					break;
				case 1:
					numberIndicator.numberIndicator.setText("早餐前测量");
					break;
				case 2:
					numberIndicator.numberIndicator.setText("早餐后测量");
					break;
				case 3:
					numberIndicator.numberIndicator.setText("午餐前测量");
					break;
				case 4:
					numberIndicator.numberIndicator.setText("午餐后测量");
					break;
				case 5:
					numberIndicator.numberIndicator.setText("晚餐前测量");
					break;
				case 6:
					numberIndicator.numberIndicator.setText("晚餐后测量");
					break;
				case 7:
					numberIndicator.numberIndicator.setText("睡前\n测量");
					break;
				default:
					numberIndicator.numberIndicator.setText("随机\n测量");
					break;
				}
			}
